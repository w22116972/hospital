...
#import "detailClassTVC.h"
#import "detailByTab1.h"
#define REFRESH_HEADER_HEIGHT 52.0f

@interface detailClassTVC ()

@end

@implementation detailClassTVC

@synthesize textPull, textRelease, textLoading, refreshHeaderView, refreshLabel, refreshArrow, refreshSpinner,strClass,CLASS,sqlClass,latitude,longitude,locationManager,currentLocation;
...
- (void)viewDidLoad {
   ...
   // currentLocation = [[CLLocation alloc]init];
    ...
    //sqlClass = [[NSString alloc]init];
  //  NSLog(@"%@",sqlClass);
    
    strClass = @[@"medical",@"surgical",@"five",@"woman",@"child",@"teeth"];
  //  NSArray *strSym = @[@"急性冠心症",@"重大外傷",@"腦中風",@"嚴重敗血症",@"主動脈剝離"];
   // whatData = [[NSMutableArray alloc]initWithObjects:strClass,strSym,nil];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  //  NSLog(@"cell");
    appambuClass *rs=(appambuClass *)[sortedArray objectAtIndex:indexPath.row];
    // NSLog(@"after cell");
...
-(void) readDatabaseForClass{
    sqlite3 *database;
    appuser = [[NSMutableArray alloc] init];
   // NSLog(@"read");
    
    if(sqlite3_open([databasePath UTF8String], &database)==SQLITE_OK) {
        NSString *strSql = [NSString stringWithFormat:@"select * from appambu natural join appstatus natural join appusers where %@appambu.id=appstatus.id AND appusers.hospital_PK=appstatus.id",sqlClass];
		...
		if(sqlite3_prepare_v2(database, sqlStatement, -1, &compiledStatement, NULL) == SQLITE_OK) {
            ...
            while(sqlite3_step(compiledStatement) == SQLITE_ROW) {
                ...
                // color = 13
                //hos_pk = 17 ,addr = 21
                NSString *Id = [NSString stringWithCString:(char *)sqlite3_column_text(compiledStatement, 0) encoding:NSUTF8StringEncoding];
				NSString *Name = [NSString stringWithCString:(char *)sqlite3_column_text(compiledStatement, 1) encoding:NSUTF8StringEncoding];
                NSString *Addr = [NSString stringWithCString:(char *)sqlite3_column_text(compiledStatement, 21) encoding:NSUTF8StringEncoding];
            //    NSLog(@"%@",Name);
                double Dis = sqlite3_column_double(compiledStatement, 16);
                NSString *Col =[NSString stringWithCString:(char *)sqlite3_column_text(compiledStatement,13) encoding:NSUTF8StringEncoding];
                NSString *Tel = [NSString stringWithCString:(char *)sqlite3_column_text(compiledStatement,20) encoding:NSUTF8StringEncoding];
                NSString *Wait = [NSString stringWithCString:(char *)sqlite3_column_text(compiledStatement, 14) encoding:NSUTF8StringEncoding];
                double Bed = sqlite3_column_double(compiledStatement, 15);
                
                appambuClass *user = [[appambuClass alloc]initWithPk:Id name:Name addr:Addr tel:Tel color:Col wait:Wait bed:Bed dis:Dis];
              //  i++;
...
-(void)readDatabase{
    ...
    appuser = [[NSMutableArray alloc] init];
    if(sqlite3_open([databasePath UTF8String], &database)==SQLITE_OK) {
        NSString *strSql = [NSString stringWithFormat:@"select * from appusers natural join appstatus where '%@' > 0 AND appusers.hospital_PK=appstatus.id",];
        const char *sqlStatement = [strSql UTF8String];
		sqlite3_stmt *compiledStatement;
		if(sqlite3_prepare_v2(database, sqlStatement, -1, &compiledStatement, NULL) == SQLITE_OK) {
			// Loop through the results and add them to the feeds array
